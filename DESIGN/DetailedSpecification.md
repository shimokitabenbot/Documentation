...
###### 正常終了
```
  { "tweet" : "ツイート文章", "twittered_at" : "ツイートした日付" }
```

例
```
  { "tweet" : "[単語]: わい\n[意味]: 私。僕。俺。\n[例文]: わいがモテないのはどう考えてもおめどが悪い。\n(私がモテないのはどう考えてもお前らが悪い。)", "twittered_at" : "2014-01-31T10:00:00.111Z"}
...
|:----------:|:-----------:|:------------:|:------------:|
| データベースに単語がない | 51000004 | word_not_exist | なし |
| Twitterにアクセスできない | 52000001 | access_unabled | なし |
| ツイート失敗 | 52000002 | twitter_failed | ツイート文章 |
...
##### リクエストボディ
```
  { "word" : "検索したい単語", match_type: "検索条件" }
```
例
```
  { "word" : "わい", match_type: "part" }
```
...
| エラー内容 | エラーコード | エラーメッセージ | エラー詳細 |
|:----------:|:-----------:|:------------:|:------------:|
| 検索対象単語が指定されていない | 11000003 | no_word | なし |
